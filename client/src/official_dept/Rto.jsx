...
const RTODashboard = () => {
  ...
  useEffect(() => {
    ...
    newSocket.on('new_grievance_assigned', (grievance) => {
      ...
      fetchGrievances(); // Refresh grievances to include the new assignment
    });
    ...
    newSocket.on('grievance_status_updated', ({ grievanceId, newStatus }) => {
      ...
      fetchGrievances(); // Refresh grievances to reflect the new status
    });
  ...
  const fetchGrievances = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/grievances/official/${user.id}?department=RTO`, {
        headers: {
      ...
      const organized = {
        pending_acceptance: data.filter(g => g.status === 'Pending'),
        assigned: data.filter(g => g.status === 'Assigned' && g.assignedOfficer === user.id),
        inProgress: data.filter(g => g.status === 'In Progress' && g.assignedOfficer === user.id),
        resolved: data.filter(g => g.status === 'Resolved' && g.assignedOfficer === user.id),
        closed: data.filter(g => g.status === 'Rejected' && g.assignedOfficer === user.id)
      };
